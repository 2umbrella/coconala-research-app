...
- 初期値はソースコード（`app.py` の `DEFAULT_ACCESS_CODE`）に書かれています
- **リポジトリを公開する場合や、合言葉を変えたい場合は必ずSecretsを使ってください**
  Streamlit Cloud のアプリ設定 → Settings → Secrets に次を貼り付けて保存します：

  ```toml
  access_code = "任意の数字"
  ```

  Secretsに登録するとそちらが優先され、合言葉はソースコードに残りません。
  保存すると自動でアプリが再起動します。
